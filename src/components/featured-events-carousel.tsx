...
export function FeaturedEventsCarousel() {
  ...
  useEffect(() => {
    const selectedFeatured = MOCK_EVENTS_DATA.filter(event => (event.averageRating || 0) >= 4.0 || new Date(event.date) > new Date())
                                           ...
                                           .slice(0, 7); // Show more featured events
  ...
  return (
    <section className="mb-12 md:mb-16">
      ...
      <Swiper
        ...
        className="relative rounded-2xl shadow-2xl overflow-hidden group" // More pronounced shadow and rounding
      >
        {featuredEvents.map((event, index) => (
          <SwiperSlide key={event.id}>
            <Link href={`/events/${event.id}`} className="block aspect-[16/9] md:aspect-[20/9] w-full h-full relative group/slide">
              <Image
                src={event.imageUrl || `https://picsum.photos/seed/${event.id}/1200/550`}
                alt={event.name}
                fill
                sizes="(max-width: 768px) 100vw, (max-width: 1200px) 80vw, 1200px" // Optimized sizes
                className="object-cover transition-transform duration-700 ease-in-out group-hover/slide:scale-110" // Slower, smoother zoom
        ...
        <div className="swiper-button-prev-custom-featured absolute top-1/2 left-4 z-10 transform -translate-y-1/2 p-2.5 bg-black/20 text-white rounded-full cursor-pointer hover:bg-black/40 transition-all opacity-0 group-hover:opacity-100 glassmorphism-dark focus-visible:ring-2 focus-visible:ring-white">
            <ChevronLeft size={30} />
        </div>
        <div className="swiper-button-next-custom-featured absolute top-1/2 right-4 z-10 transform -translate-y-1/2 p-2.5 bg-black/20 text-white rounded-full cursor-pointer hover:bg-black/40 transition-all opacity-0 group-hover:opacity-100 glassmorphism-dark focus-visible:ring-2 focus-visible:ring-white">
            <ChevronRight size={30} />
        </div>
        {/* Custom Pagination Container */}
        <div className="swiper-pagination-custom-featured absolute bottom-5 left-1/2 transform -translate-x-1/2 z-10 flex space-x-2.5"></div>
      </Swiper>
